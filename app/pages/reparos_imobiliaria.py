...
def criar_layout_reparos_imobiliaria(df: pd.DataFrame):

    df_f = df[df['Finalizado'] == True].copy()

    df_f['Data_inicio_desocupacao'] = pd.to_datetime(
    ...
    def criar_grafico(df_base: pd.DataFrame, opacidade_total=0.85, opacidade_imobiliaria=0.85):
        reparos_mensal = (
            df_base
            .groupby('mes_ano', as_index=False)
            .agg(
                **{
                    'Fechou com Imobiliária': ('fechou_com_imobiliaria', 'sum'),
        ...
        reparos_mensal['Percentual'] = (
            reparos_mensal['Fechou com Imobiliária'] /
        ...
        total_fechou = df_base['fechou_com_imobiliaria'].sum()
        percentual_geral = (
        ...
        fig.add_bar(
            x=reparos_mensal['Mês/Ano'],
            y=reparos_mensal['Fechou com Imobiliária'],
            name='Inquilino Fechou com Imobiliária',
            opacity=opacidade_imobiliaria,
            marker=dict(color='#228B22', line=dict(color='black', width=1.3)),
            text=reparos_mensal['Fechou com Imobiliária'],
            textposition='outside',
            textfont=dict(size=12, color='black'),
            hovertemplate='%{x}, %{y}<extra></extra>'
        )
        ...
        fig.add_annotation(
            ...
            text=(
                ...
                f"<b>Fechou com Imobiliária:</b> {total_fechou}<br>"
    ...
    layout = html.Div(
        children=[
            ...
            html.Button(
                "📥",
                id="btn-download-reparos-imobiliaria",
                style={
                    "position": "fixed",
                    "top": "220px",
                    "right": "14px",
            ...
            dcc.Download(id="download-reparos-imobiliaria"),
    
            dcc.Graph(
                id='grafico-reparos-imobiliaria',
                config={'displayModeBar': True, 'displaylogo': False}
            ),

            dash_table.DataTable(
                id='tabela-reparos-imobiliaria',
                columns=[{'name': col, 'id': col} for col in COLUNAS_TABELA_REPAROS_IMOB],
                data=[],
                page_size=15,
